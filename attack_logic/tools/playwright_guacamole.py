    ...
    with sync_playwright() as p:
        ...
        for i in range(20):
            ...
            if page.locator("text=EWS").count() > 0:
                ...
                page.locator("text=EWS").first.click()
                ews_clicked = True
                break

        if not ews_clicked:
            print("[-] Could not find EWS connection tile after 20 iterations")
            browser.close()
            sys.exit(1)

        # Signal ready — VNC session initiated
        log_time("VNC Session Active. Waiting for attacker actions...")
        with open("/tmp/guac_ready.txt", "w") as f:
            f.write("ready")

        # Keep session alive with Playwright-aware waits so WebSocket ACKs are processed
        for _ in range(60):
            if os.path.exists(stop_file):
